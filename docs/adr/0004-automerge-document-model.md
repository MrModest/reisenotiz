...
Each user has a `RootDoc` (a `tripIndex` of trip URLs, plus `userAirports` and `userAccommodations`), and each trip is a separate `TripDoc` holding the trip and its items. The root document's URL is persisted in `localStorage`; trip URLs are discovered through the index rather than stored individually.
